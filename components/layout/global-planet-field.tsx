...
> = {
  home: {
    near: ["#27151b", "#9d4735", "#291822"],
    middle: ["#162b29", "#5d8c58", "#152625"],
    far: ["#111a38", "#4969a2", "#12162d"],
    light: 0xbfe7df,
    starScale: 1.12,
  },
  about: {
    near: ["#2c1717", "#b56845", "#351b24"],
    middle: ["#34241d", "#d39a63", "#2d1d1c"],
    far: ["#261a31", "#87648f", "#171326"],
    light: 0xffc89d,
    starScale: 0.82,
  },
  projects: {
    near: ["#111d2e", "#315a78", "#0d1524"],
    middle: ["#102b2b", "#55a28e", "#0a2020"],
    far: ["#20223d", "#7180c2", "#11152c"],
    light: 0x9fd9ff,
    starScale: 1.28,
  },
  docs: {
    near: ["#171d19", "#48564b", "#111512"],
    middle: ["#253127", "#77906d", "#172019"],
    far: ["#1b2430", "#53677b", "#10161e"],
    light: 0xdde8d8,
    starScale: 0.5,
  },
  playground: {
    near: ["#23132d", "#7a3b87", "#161022"],
    middle: ["#0d3031", "#33a99d", "#081c22"],
    far: ["#2a1931", "#d35d79", "#15101f"],
    light: 0xb8fff0,
    starScale: 1.45,
  },
  observatory: {
    near: ["#151c2b", "#405c82", "#101521"],
    middle: ["#28311b", "#8aa653", "#171d13"],
    far: ["#30231b", "#b37a4e", "#1a1411"],
    light: 0xc7dcff,
    starScale: 0.88,
  },
};
...
export function GlobalPlanetField() {
  ...
  useEffect(() => {
    ...
    import("three").then((THREE) => {
      ...
      renderer.setPixelRatio(
        Math.min(window.devicePixelRatio, quality === "high" ? 1.2 : 1),
      );
      ...
      const makeStarLayer = (
        ...
        for (let index = 0; index < count; index += 1) {
          ...
          positions[offset + 1] = (random() - 0.5) * spread * 0.68;
      ...
      const density =
        (quality === "high" ? 1 : quality === "balanced" ? 0.58 : 0.25) *
        environment.starScale;
      const farStars = makeStarLayer(
        Math.round(720 * density),
        34,
        -20,
        0.035,
        0.42,
      ...
      const makeTexture = (
        ...
        for (let y = 0; y < 256; y += rocky ? 5 : 3) {
          const offset = Math.sin(y * 0.05 + seed) * 18;
          context.fillStyle = `rgba(255,255,255,${rocky ? 0.025 + random() * 0.05 : 0.045 + random() * 0.09})`;
        ...
        for (let index = 0; index < details; index += 1) {
          ...
          context.fillStyle = rocky
            ? `rgba(12,18,18,${0.05 + random() * 0.15})`
            : `rgba(255,220,185,${0.025 + random() * 0.06})`;
          context.fill();
        }
      ...
      const makePlanet = (
        radius: number,
        texture: ReturnType<typeof makeTexture>,
        ringed = false,
      ) => {
        ...
        const atmosphere = new THREE.Mesh(
          new THREE.SphereGeometry(
            radius * 1.035,
            Math.max(16, detail[0] - 8),
            Math.max(12, detail[1] - 6),
          ),
        ...
        if (ringed) {
          for (let band = 0; band < (quality === "low" ? 3 : 7); band += 1) {
            const inner = radius * (1.34 + band * 0.08);
            const ring = new THREE.Mesh(
              ...
              new THREE.MeshBasicMaterial({
                color: band % 2 ? 0xc9ff57 : 0xe5cfa8,
                transparent: true,
                opacity: 0.14 + band * 0.025,
      ...
      near.rotation.z = -0.16;
      const middle = makePlanet(
        1.85,
        makeTexture(271, environment.middle, false),
        true,
      );
      middle.position.set(7.5, -0.8, -3.8);
      middle.rotation.set(0.18, -0.45, 0.1);
      const far = makePlanet(0.72, makeTexture(811, environment.far, false));
      ...
      if (environmentKey === "about") {
        [
          { radius: 0.17, distance: 2.55, color: 0xffc78f, phase: 0.35 },
          { radius: 0.1, distance: 3.05, color: 0xeaa6b8, phase: 2.4 },
      ...
      if (environmentKey === "docs") {
        [1.05, 1.28, 1.55].forEach((radius, index) => {
          const reticle = new THREE.Mesh(
            new THREE.RingGeometry(radius, radius + 0.012, 72),
            new THREE.MeshBasicMaterial({
              color: 0xdde8d8,
              transparent: true,
          ...
          reticle.rotation.x = Math.PI * 0.52;
          far.add(reticle);
        });
      }
      ...
      if (environmentKey === "playground") {
        [4.6, 5.35, 6.1].forEach((radius, index) => {
          const laboratoryOrbit = new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({
              color: index === 1 ? 0xc9ff57 : 0x8ce6e6,
              transparent: true,
              opacity: 0.12,
            }),
          );
          laboratoryOrbit.rotation.set(1.1 + index * 0.17, 0.2, index * 0.38);
      ...
      const scheduleRender = () => {
        if (!scheduled) scheduled = window.requestAnimationFrame(render);
      };
      let nearBaseY = -6.1;
      let middleBaseY = -0.8;
      let farBaseY = 4.2;
      ...
      const pointer = (event: PointerEvent) => {
        ...
        system.rotation.y = pointerX * 0.055;
        system.rotation.x = pointerY * -0.035;
        nearStars.position.set(pointerX * -0.22, pointerY * 0.16, 0);
        middleStars.position.set(pointerX * -0.1, pointerY * 0.07, 0);
        farStars.position.set(pointerX * -0.035, pointerY * 0.025, 0);
        scheduleRender();
      };
      const scroll = () => {
        const offset = window.scrollY * 0.00045;
        near.position.y = nearBaseY + offset;
        middle.position.y = middleBaseY + offset * 0.45;
        far.position.y = farBaseY + offset * 0.18;
        nearStars.position.y = window.scrollY * 0.00016;
        middleStars.position.y = window.scrollY * 0.00007;
        (near.userData.sphere as InstanceType<typeof THREE.Mesh>).rotation.y =
          window.scrollY * 0.00012;
        (middle.userData.sphere as InstanceType<typeof THREE.Mesh>).rotation.y =
          -window.scrollY * 0.00018;
        scheduleRender();
      };
  ...
  return (
    <canvas
      key={pathname}
      className={`global-planet-field environment-${environmentKey}`}
      ref={canvasRef}
      aria-hidden="true"
    />
  );
