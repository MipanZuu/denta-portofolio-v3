...
export async function createBlogCommentAction(
  ...
  if (authorName.length < 2 || authorName.length > 50)
    return { error: "Your name must be between 2 and 50 characters." };
  if (!content || content.length > 1000)
    return { error: "Your comment must be between 1 and 1,000 characters." };
  ...
  const [recent] = await db
    ...
    .where(
      and(
        eq(blogComments.visitorId, visitorId),
        gt(blogComments.createdAt, cutoff),
    ...
    .limit(1);
  if (recent)
    return { error: "Please wait a few seconds before commenting again." };

  await db
    .insert(blogComments)
    .values({ blogPageId, visitorId, authorName, content });
  updateTag(BLOG_CACHE_TAGS.feed);
