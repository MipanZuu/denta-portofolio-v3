...
export async function generateMetadata({
  params,
}: PageProps<"/blog/[blogSlug]">): Promise<Metadata> {
  ...
  const socialImage = post.coverImageUrl ?? media[0] ?? seo.defaultOgImage;
...
export default async function BlogPostPage({
  ...
  const [comments, engagement] = await Promise.all([
    ...
    getBlogEngagement(post.id, visitorId),
  ]);
  ...
  return (
    <main className="blog-detail-page page-shell">
      ...
      <article className="blog-detail-card">
        <header>
          <BlogAuthor date={formatBlogDate(post.publishedAt)} />
        ...
        <div className="blog-detail-actions">
          ...
          <a
            ...
            href="#comments"
            aria-label={`${engagement.commentCount} comments`}
          >
            <CommentIcon />
          ...
          <ViewCounter
            blogPageId={post.id}
            slug={post.slug}
            initialCount={engagement.viewCount}
            track
          />
        </div>
        <div className="blog-detail-intro" id="article">
          <p className="eyebrow">
            {formatBlogDate(post.publishedAt)} · Field note
          </p>
          <h1>{post.title}</h1>
          <p>{post.excerpt}</p>
          {tags.length ? (
        ...
        <div className="blog-article-content">
          ...
          {post.secondaryContent ? (
            <section>
              <ContentBlock content={post.secondaryContent} />
            </section>
          ) : null}
          {post.thirdContent ? (
            <section>
              <ContentBlock content={post.thirdContent} />
            </section>
          ) : null}
        </div>
        <section className="blog-comments" id="comments">
          ...
          <CommentForm blogPageId={post.id} slug={post.slug} />
        </section>
      </article>
    </main>
  );
